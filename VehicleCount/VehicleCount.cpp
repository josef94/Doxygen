/*! \file VehicleCount.cpp
    \brief The main file for the post processing.
	
    This class is responsible for the post processing of the pictures. The post-processing links the single frames to find out a vehicle driver and calculate some properties which are saved into a file. If all vehicle drivers are processed, the class deletes the folder with the single frames.
*/
...
// global variables ///////////////////////////////////////////////////////////////////////////////
    //! A public variable which holds the name of the picture.
    /*!
      This string variable is important to hold the current name of the picture.
    */
	string pictureName = "crop%03d.tif";
	
	//! A public variable which holds the path to the crops folder.
    /*!
      This string variable defines the path to the crops folder which is necessary to save all the generated crops of a vehicle driver.
    */
	string pathSaveROIPicture = "/BA/Crops/";
	
	//! A public variable which holds the path to the feature vector.
    /*!
      This string variable defines the name and path of the feature vector.
    */
	char fileName[256] = "/BA/FeatureVec.csv";
	
	//! A public variable which holds the actual folder.
    /*!
      This string variable holds the folder name which was passed as argument. This is important to know which frames must be processed now.
    */
	string folder;

	//! A public variable which holds all filenames.
    /*!
      This is a vector of strings which contains all filenames there are in the specified folder.
    */
	vector<String> filenames;

	//! A public variable which holds a counter.
    /*!
        This integer variable holds the number of relevant pictures there are for the one current vehicle driver.
    */
	int countRelevantPictures = 0;

	//! A public variable which is used as distance between two pictures.
    /*!
      This integer variable holds the amount distance between two frames where it counts as the same vehicle driver. If the distance is greater than this amount, the program knows to start a new vehicle driver.
    */
	int distanceThresh = 5;

	//! A public variable which holds a counter for the vehicles passed from left to right.
    /*!
      This integer variable is used as counter and increases if a vehicle is tracked from left to right.
    */
	int carCountLTR = 0;

	//! A public variable which holds a counter for the vehicles passed from right to left.
    /*!
      This integer variable is used as counter and increases if a vehicle is tracked from right to left.
    */
	int carCountRTL = 0;

	//! A public variable as boolean that indicates if the vehicle was already calculated.
    /*!
      This variable is used to know if the current vehicle was already counted to the specified counter.
    */
	bool calculated = false;

// function prototypes ////////////////////////////////////////////////////////////////////////////
    //! This function is used to delete the current directory if the process is finished.
    /*!
      \param deletePath Path which has to be deleted.
    */
	void deleteDir(string deletePath);

    //! This function is used to copy a folder as zip to the download folder and delete it after this.
    /*!
      \param deletePath Path which must be deleted after copying it to the download folder as zip.
	  \param folder The name of the new folder in the download folder.
    */
	void copyDir(string deletePath, string folder);

    //! This function is used to matches two input blobs.
    /*!
      \param existingBlobs The Blob which is already tracked.
	  \param currentFrameBlobs The Blob of the current frame.
    */
	void matchCurrentFrameBlobsToExistingBlobs(vector<Blob> &existingBlobs, vector<Blob> &currentFrameBlobs);

	//! This function is used to matches two input blobs.
    /*!
      \param currentFrameBlob The blob of the current frame.
	  \param existingBlobs The blob which is already tracked.
	  \param intIndex The integer of the current blob.
    */
	void addBlobToExistingBlobs(Blob &currentFrameBlob, vector<Blob> &existingBlobs, int &intIndex);
	
	//! This function is used to add a new blob to the existing blobs.
    /*!
      \param currentFrameBlob The blob of the current frame.
	  \param existingBlobs The blob which is already tracked.
    */
	...
	//! This function is used to calculate the distance between two input points.
    /*!
    ...
    */
	...
	//! This function is used to check if a blob crossed the line.
    /*!
      \param blobs The blob which has to be checked.
	  \param intHorizontalLinePosition The position of the line.
	  \return A number dependent of the result.
    */
	int checkIfBlobsCrossedTheLine(vector<Blob> &blobs, int &intHorizontalLinePosition);
	
	//! This function is used to save a vehicle to the feature vector.
    /*!
      \param blobs The blob which is used to calculate the crop dimensions.
	  \param imgFrame2Copy The frame which is used for the crop, the original picture and the blur.
	  \param imgFrame2Copy2 The frame which is used for the blur.
	  \param k The index from frame to calculate the correct timestamp.
	  \param direction A bool if the direction was from right to left or from left to right.
	  \param vehicleCount A variable carCountLTR or carCountRTL dependant of the direction.
	  \param relevantPictures A counter how many pictures were involved for this vehicle driver.
	  \return The number dependant of the result.
    */
	...
	//! This function is used to get the specified number from the file name.
    /*!
      \param name The name of the current picture.
	  \param i The integer of the current picture name.
	  \return The number as integer from the current picture.
    */
